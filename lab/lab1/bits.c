...
int fitsBits(int x, int n) {
  // return !((x >> (n - 1)) & (1 << 31 >> 31)) | !((~(x >> (n - 1))) & (1 << 31 >> 31));
  return !(x << (32 + (~n + 1)) >> (32 + (~n + 1)) ^ x) && n != 32;
}
/*
...
int addOK(int x, int y) {
  return !((x >> 31) ^ ((x + y) >> 31)) | !((y >> 31) ^ ((x + y) >> 31));
}
/*
...
int invert(int x, int p, int n) {
  return ((1 << 31) >> (31 + (~p + 1))) ^ ((1 << 31) >> (31 + (~p + 1) + (~n + 1))) ^ x ;
}
// Rating: 4
/*
